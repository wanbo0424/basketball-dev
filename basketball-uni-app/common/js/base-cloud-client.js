...
function call({ url, data = {}, success, fail, complete , requestFail , debug , cacheKey , cacheSpace , keepStr = false , keepKeys }) {
	...
	if (isDebug(debug) ) {
		console.log("");
		console.log("request >>>>>>>>>>>>>>>>");
		console.log(`开始请求，云函数：${name}/${action}，参数：`) ;
...
function isDebug(debug){
	return ( getApp().globalData.debug === true || debug === true ) &&  process.env.NODE_ENV === 'development' ;
}
...
function callSuccess(data, success, fail) {
	...
	if (data.state == "noAuth") {
		uni.showModal({
			...
			showCancel: false,
			success: function(e) {
				var firstSubPages =  getApp().$store.getters["window/getFirstPages"] ;
				if (!!firstSubPages) {
					uni.redirectTo({
						url: firstSubPages
					});
...
function clearStorage(e) {
	...
	uni.removeStorageSync("menuGroups");
